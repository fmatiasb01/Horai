...
def mostrar_chat(request: Request, sesion_id: str = None, db: Session = Depends(get_db)):
    if not sesion_id:
        ...
        conversaciones_recientes = obtener_todas_las_conversaciones(db)
        if conversaciones_recientes:
            sesion_id = conversaciones_recientes[0].sesion_id
        else:
...
def enviar_mensaje(request: Request, mensaje_usuario: str = Form(...), sesion_id: str = Form(...), db: Session = Depends(get_db)):
    respuesta_bot = f"HORAI responde: {mensaje_usuario[::-1]}"
    crear_conversacion(db, mensaje_usuario, respuesta_bot, sesion_id)
